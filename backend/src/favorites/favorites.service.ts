...
export class FavoritesService {
  ...
  async getFavoriteQuestions(userId: string) {
    const favoriteQuestions =
      await this.prismaService.questionFavorite.findMany({
        where: { userId },
  ...
  async unfavoriteQuestion(userId: string, questionId: string) {
    const favoriteQuestion =
      await this.prismaService.questionFavorite.findUnique({
        where: {
          userId_questionId: { questionId: questionId, userId: userId },
        },
      });
  ...
  async getFavoriteStudyGuides(userId: string) {
    const favoriteStudyGuides =
      await this.prismaService.studyGuideFavorite.findMany({
        where: { userId },
      });
  ...
  async unfavoriteStudyGuide(userId: string, studyGuideId: string) {
    const favoriteStudyGuide =
      await this.prismaService.studyGuideFavorite.findUnique({
        where: {
          userId_studyGuideId: { studyGuideId: studyGuideId, userId: userId },
        },
      });
