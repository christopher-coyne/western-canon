...
export class StudyGuidesService {
    ...
    async updateStudyGuide({data, userId, id}: {data: CreateStudyGuideDto, userId: string, id: string}) {
        const studyGuide = await this.prismaService.studyGuide.findUnique({
            where: {id}
        })
    ...
    async createSectionForStudyGuide({data, userId, studyGuideId}: {data: CreateStudyGuideSectionDto, userId: string, studyGuideId: string}) {
        const studyGuide = await this.prismaService.studyGuide.findUnique({
