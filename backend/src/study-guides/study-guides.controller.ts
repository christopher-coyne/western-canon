...
export class StudyGuidesController {
  ...
  async createStudyGuide(@Req() req, @Body() body: CreateStudyGuideDto) {
    ...
    return Result.ok(await this.studyGuidesService.createStudyGuide(body, userId));
  }
  ...
  @UseGuards(AuthenticatedGuard)
  @Post('/:id/sections')
  async createSectionForStudyGuide(@Req() req, @Param('id') id: string, @Body() body: CreateStudyGuideSectionDto) {
    const userId = req.user.id
    return Result.ok(await this.studyGuidesService.createSectionForStudyGuide({userId, data: body, studyGuideId: id}));
  }
}