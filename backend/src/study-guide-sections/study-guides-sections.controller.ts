...
export class StudyGuidesController {
  constructor(private readonly studyGuidesSectionsService: StudyGuidesSectionsService) {}
  ...
  @Post('/:id/question')
  async createQuestion(@Req() req, @Param('id') id: string, @Body() body: CreateStudyGuideQuestionDto) {
    const userId = req.user.id
    return Result.ok(await this.studyGuidesSectionsService.createQuestion({data: body, sectionId: id, userId: id}));
  }
